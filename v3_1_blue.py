...
def lgb_random_search(X_train_trans, y_train, n_iter=16, random_state=RANDOM_STATE):
    ...
    # 超参数搜索空间 - 增加正则化强度，限制模型复杂度
    param_dist = {
        'num_leaves': [7, 15, 23],  # 减少叶子节点数上限
        'learning_rate': [0.02, 0.03, 0.04],  # 降低学习率
        'n_estimators': [100, 150, 200, 250],  # 适当减少树的数量
        'max_depth': [2, 3, 4],
        'min_child_samples': [40, 60, 80, 100],  # 增加最小子样本数
        'subsample': [0.6, 0.7, 0.8],  # 减少子采样比例
        'colsample_bytree': [0.5, 0.6, 0.7],  # 减少特征采样比例
        'reg_alpha': [1.0, 2.0, 4.0, 8.0],  # 增加L1正则化强度
        'reg_lambda': [2.0, 4.0, 6.0, 8.0],  # 增加L2正则化强度
        'min_split_gain': [0.1, 0.2, 0.3, 0.4]  # 增加最小分割增益
    }
